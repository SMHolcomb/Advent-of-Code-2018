...
def calc_change(freq_changes):

	curr_frequency = 0
	for i in range(0,len(freq_changes)):
		curr_frequency += freq_changes[i]

	return curr_frequency


def calc_change2(freq_changes):

	curr_frequency = 0
	running_frequency = [curr_frequency , ]

	while True: 
		for i in range(0,len(freq_changes)):
			curr_frequency += freq_changes[i]
			
			if curr_frequency in running_frequency:
				return curr_frequency
		
			running_frequency.append(curr_frequency)
...
def main():
	...
	with open('frequency_inputs.txt','r') as frequency_file:
		for value in frequency_file:

			freq_changes.append(int(value))

	
	# Part I
	curr_frequency = calc_change(freq_changes)
	print(curr_frequency)

	#Part II:
	#freq_changes = [+1, -2, +3, +1] #testing
	first_dup = calc_change2(freq_changes)
	print(first_dup)
